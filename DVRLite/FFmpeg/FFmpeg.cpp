...
namespace DVRLite
{
    ...
    void FFmpeg::Record(std::chrono::seconds duration, const std::filesystem::path& path)
    {
        ...
        if (!isRecording)
        {
            Log(filter, "Begin Record " + source.GetName());
            if (recordThread.joinable())
                recordThread.join();
            isRecording = true;
        ...
        {
            Log(filter, "Extend Record " + source.GetName());
            endTime = std::chrono::system_clock::now() + duration;
        }
    }
    ...
    bool FFmpeg::InitialiseRecordThread(const char* in_filename, const char* out_filename, RecordThreadContext& context) const
    {
        ...
        {
            Log(filter, "Retrieve input stream info failed.");
            return false;
        }
        ...
        if (!context.ofmt_ctx)
        {
            Log(filter, "Create output context failed.");
            return false;
        }
        ...
        if (!(context.ofmt->flags & AVFMT_NOFILE))
        {
            ...
            {
                Log(filter, std::string("Open output file ") + out_filename + " failed.");
                return false;
            }
        }
        ...
        {
            Log(filter, "Error occurred when opening output file.");
            return false;
        }
        return true;
    }
    ...
    bool FFmpeg::MapInput(RecordThreadContext& context, StreamMap& streamMap, AVStream* stream)
    {
        ...
        if (!streamMap.out)
        {
            Log(filter, "Allocate output stream failed.");
            return false;
        }
        ...
        {
            Log(filter, "Failed to copy context from input to output stream codec context.");
            return false;
        }
    ...
    {
        Log(filter, "RecordThread Begin " + source.GetName());
        ...
        {
            Log(filter, "Initialise Record Failed " + source.GetName());
            isRecording = false;
            return;
        }
        ...
        while (true)
        {
            ...
            if (current_stream_map != nullptr)
            {
                ...
                {
                    Log(filter, "Error muxing packet.");
                    continue;
                }
    ...
    {
        Log(filter, "End record" + source.GetName());
        // Write file trailer
