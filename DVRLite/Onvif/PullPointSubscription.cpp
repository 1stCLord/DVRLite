...
PullPointSubscription::PullPointSubscription(const DVRLite::Source& source, std::function<void(void)> alert) :
	running(true),
	alert(alert),
	source(source)
{
...
{
	DeviceBindingProxy deviceBinding = DeviceBindingProxy(source.address.c_str());
	soap_register_plugin(deviceBinding.soap, soap_wsse);

	soap_wsse_add_UsernameTokenDigest(deviceBinding.soap, NULL, source.username.c_str(), source.password.c_str());
	...
	soap_wsse_add_UsernameTokenDigest(pullpointSubscriptionBindingProxy->soap, NULL, source.username.c_str(), source.password.c_str());
